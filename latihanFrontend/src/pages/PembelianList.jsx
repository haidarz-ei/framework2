...
function TambahPembelianModal({ onClose, onSuccess }) {
  ...
  const emptyItem = () => ({
    ...
    harga_beli: '',
    isNew: false,
    // field names disesuaikan dengan project (camelCase)
    new_produk: { kodeBarang: '', namaBarang: '', kategori: '', harga: '' }
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resSuppliers, resProduks] = await Promise.all([
          api.get('/suppliers'),
          api.get('/produks')
        ]);
        setSuppliers(resSuppliers.data.data || resSuppliers.data || []);
        setProduks(resProduks.data.data || resProduks.data || []);
      } catch {
        toast.error('Gagal memuat data supplier atau produk');
      }
    };
    fetchData();
  }, []);
  ...
  const updateItem = (idx, field, value) => {
    ...
    if (field === 'produk_id') {
      if (value === '__new__') {
        ...
        // Produk project pakai field "harga" (camelCase sudah sama)
      ...
      updated[idx] = {
        ...updated[idx],
        new_produk: { ...updated[idx].new_produk, [subField]: value }
      };
  ...
  const totalHarga = items.reduce(
    (sum, i) => sum + (parseInt(i.quantity) || 0) * (parseFloat(i.harga_beli) || 0), 0
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.post('/pembelians', {
        ...form,
        items: items.map((i) => ({
          produk_id: i.produk_id === '__new__' ? null : i.produk_id,
          quantity: parseInt(i.quantity) || 0,
          harga_beli: parseFloat(i.harga_beli) || 0,
          new_produk: !i.isNew ? null : {
            // Kirim ke backend dengan field camelCase sesuai project
            kodeBarang: i.new_produk.kodeBarang || '',
            namaBarang: i.new_produk.namaBarang,
        ...
        <div className="p-8">
          ...
          <div className="mb-5">
            ...
            <div className="space-y-3">
              {items.map((item, idx) => (
                <div key={idx} className="border border-gray-200 bg-gray-50 rounded-2xl p-4">
                  <div className="flex gap-2 items-center mb-3">
                    ...
                    >
                      ...
                      <option value="__new__" className="text-orange-600 font-medium">+ Produk Baru (Belum ada di daftar)</option>
                      {produks.map((p) => (
                        <option key={p.id} value={p.id}>
                          {/* Gunakan namaBarang (camelCase sesuai project) */}
                          {p.namaBarang} (Stok: {p.stok ?? p.quantity ?? 0})
                        </option>
                      ))}
                    </select>
                    {items.length > 1 && (
                  ...
                  {item.isNew && (
                    <div className="mb-3 space-y-2 border-t border-orange-200 pt-3">
                      ...
                      <input
                        ...
                        value={item.new_produk.namaBarang}
                        onChange={(e) => updateItem(idx, 'new_produk.namaBarang', e.target.value)}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          value={item.new_produk.kodeBarang}
                          onChange={(e) => updateItem(idx, 'new_produk.kodeBarang', e.target.value)}
          ...
          <div className="mb-6">
            ...
            <div className="border rounded-xl overflow-hidden">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-100 border-b text-sm font-medium text-gray-600">
                  ...
                  {pembelian.items?.map((item) => (
                    <tr key={item.id} className="border-b text-sm hover:bg-gray-50">
                      <td className="px-4 py-3">
                        {/* Gunakan namaBarang & kodeBarang (camelCase sesuai project) */}
                        <p className="font-medium">{item.produk?.namaBarang || '-'}</p>
                        <p className="text-xs text-gray-400">{item.produk?.kodeBarang || '-'}</p>
                      </td>
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-1.5 flex justify-between items-center">
          <h1
            className="text-[10px] font-medium text-gray-400 uppercase tracking-[0.2em] cursor-pointer"
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Pembelian</h2>
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-6">
          <input
            ...
            className="flex-1 md:max-w-md px-4 py-2 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-400 outline-none"
          />
          <select
            value={statusFilter}
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-2xl p-16 text-center shadow-sm">
            ...
            <p className="text-gray-500">Silakan tambahkan data pembelian baru.</p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b text-sm font-semibold text-gray-600 tracking-wider">
                    <th className="px-6 py-4">No. Pembelian</th>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`text-xs px-3 py-1 rounded-full font-semibold ${s.cls}`}>
                            {s.text}
                          </span>
                        </td>
                      </tr>
